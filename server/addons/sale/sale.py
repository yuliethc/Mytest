...
class sale_order(osv.osv):
    ...
    _columns = {
        ...
        'pricelist_id': fields.many2one('product.pricelist', 'Pricelist', required=True, select=True, readonly=True, states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, help="Pricelist for current sales order."),
        ...
        'amount_total': fields.function(_amount_all_wrapper, digits_compute=dp.get_precision('Total'), string='Total',
            store={
                'sale.order': (lambda self, cr, uid, ids, c={}: ids, ['order_line'], 10),
                'sale.order.line': (_get_order, ['price_unit', 'tax_id', 'discount', 'product_uom_qty'], 10),
            },
        ...
        'x_telefono': fields.char('Telefono', size=15, select=True),
        'x_recogido': fields.char('Nombre asistido',size=40, select=True),
        'x_vehiculo': fields.char('Vehiculo',size=40, select=True),
        'x_matricula': fields.char('Matricula',size=10, select=True),
        'x_comentarios': fields.text('Comentarios', select=True),
#        'x_imagen' : fields.Many2many('ir.attachment', select=True),
    }
    ...
    def _prepare_invoice(self, cr, uid, order, lines, context=None):
        ...
        invoice_vals = {
            ...
            'x_telefono': order.x_telefono,
            'x_recogido': order.x_recogido,
            'x_vehiculo': order.x_vehiculo,
            'x_matricula': order.x_matricula,
            'x_comentarios': order.x_comentarios,
	    'pricelist_id': order.pricelist_id.id,
	    'x_categ_id': order.x_categ_id.id,
	    'x_remolque': order.x_remolque,
            'allowed_products': order.allowed_products,
	    'x_total': order.amount_total,
        }
...
class sale_order_line(osv.osv):
    ...
    _columns = {
        ...
        'invoice_lines': fields.many2many('account.invoice.line', 'sale_order_line_invoice_rel', 'order_line_id', 'invoice_id', 'Invoice Lines', readonly=False, copy=False),
        ...
        'product_uom': fields.many2one('product.uom', 'Unit of Measure ', required=True, readonly=False, states={'draft': [('readonly', False)]}),
        'product_uos_qty': fields.float('Quantity (UoS)' ,digits_compute= dp.get_precision('Product UoS'), readonly=False, states={'draft': [('readonly', False)]}),
        'product_uos': fields.many2one('product.uom', 'Product UoS'),
        'discount': fields.float('Discount (%)', digits_compute= dp.get_precision('Discount'), readonly=False, states={'draft': [('readonly', False)]}),
        'th_weight': fields.float('Weight', readonly=False, states={'draft': [('readonly', False)]}),
        'state': fields.selection(
...
#	context_order = {'lang': lang, 'order_id': order_id}
...
#	order_obj = order_obj.browse(cr, uid, pricelist_id, context=context_order) #definicion de factura
#	result['x_accion'] = order_obj.pricelist_id
...
#            result['name'] = self.pool.get('product.pricelist').name_get(cr, uid, [pricelist], partner_id, ctx)[pricelist]
 ...
 #       price = self.pool.get('product.pricelist').price_get(cr, uid, [pricelist], product, qty or 1.0, partner_id, {'uom': uom or result.get('product_uom'), 'date': date_order,})[pricelist]
            ...
            if price is False:
                ...
                result.update({'price_unit': price, 'x_accion': pricelist})
                if context.get('uom_qty_change', False):
                    values = {'price_unit': price, 'x_accion': pricelist}
                    if result.get('product_uos_qty'):
                        values['product_uos_qty'] = result['product_uos_qty']
                    return {'value': values, 'domain': {}, 'warning': False}
#codigo mio
   #         x_accion = self.pool.get('product.pricelist').name_get(cr, uid, [pricelist],
     #                product, partner_id, {
       #              'date': date_order,
        #             })[pricelist]
