...
class account_invoice(models.Model):
    ...
    amount_total = fields.Float(string='Total', digits=dp.get_precision('Total'),
        store=True, readonly=True, compute='_compute_amount')
    ...
    check_total = fields.Float(string='Verification Total', digits=dp.get_precision('Total'),
    ...
    residual = fields.Float(string='Balance', digits=dp.get_precision('Total'),
...
#    x_recogido = fields.char('Nombre asistido',size=40, required=True),
 ...
 #   x_telefono = fields.char('Teléfono',size=15, required=True),
  #  x_imagen = fields.Many2many('ìr.attachment'),
    x_remolque = fields.Selection([
            ('Remolque','Remolque'),
            ('Rep in Situ','Rep in Situ'),
            ('Suplidos','Suplidos'),
            ('Alquiler','Alquiler'),
        ], string='Acción', index=True, default='Remolque')
    x_categ_id = fields.Many2one('product.category', 'Acción')
    x_allowed_product = fields.Many2many('product.product', string='Permitidos')
    _sql_constraints = [
...
class account_invoice_line(models.Model):
    ...
    x_accion = fields.Many2one('product.pricelist', string='Complemento')
